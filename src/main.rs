...
mod my_date_format {
        ...
        D: Deserializer<'de>,
    {
        ...
        Utc.datetime_from_str(&s[0..18], FORMAT)
...
fn get_response2(args: &Args, client: Client) -> UrlhausResponse {
    ...
    let res = client
        .post("https://urlhaus-api.abuse.ch/v1/tag")
        .form(&param)
        .send();
...
fn main() {
    ...
    if args.api.contains("threatfox") {
        ...
        let res_entries: Vec<ThreatFoxEntry> = res_json
            .data
            .into_iter()
            .filter(|e| {
            ...
            Some(x) if x.to_lowercase().eq("csv") => {
                ...
                for e in res_entries {
                    let _ = wtr.write_record(&[
                        ...
                        &e.malware_alias,
                        &e.malware_malpedia,
                        &e.confidence_level.to_string(),
                        &e.first_seen.to_string(),
                        &e.reporter,
                        &e.tags.join(":"),
                    ]);
                }
        ...
        let res_entries: Vec<UrlhausEntry> = res_json
            .urls
            .into_iter()
            .filter(|e| {
        ...
        match args.format {
            Some(x) if x.to_lowercase().eq("csv") => {
                ...
                for e in res_entries {
                    let _ = wtr.write_record(&[
                        &e.url_id,
                        &e.url.replace("http", "hxxp"),
                        &e.url_status,
                        &e.dateadded.to_string(),
                        &e.reporter,
                        &e.threat,
                        &e.tags.join(":"),
                    ]);
